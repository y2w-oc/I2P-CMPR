    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            x_proj_bias = getattr(self, "x_proj_bias", None)
            # print("Before cross scan:", x.shape, _scan_mode)
            xs = cross_scan_fn(x, in_channel_first=True, out_channel_first=True, scans=_scan_mode, force_torch=scan_force_torch)
            # what = cross_scan_fn(x, in_channel_first=True, out_channel_first=True, scans=1, force_torch=scan_force_torch)
            # print("After cross scan:", xs.shape, what.shape)
            # print((what[0,0,:,:] == what[0,1,:,:]).sum())
            if no_einsum:
            ...
            # print("Before cross merge:", ys.shape)
            ...
            # print("After cross merge:", y.shape)
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(dims, int):
            ...
            # dims = [int(dims + 32 * i_layer) for i_layer in range(self.num_layers)]
        ...
        imgsize = (imgsize[0] // topconv_scale, imgsize[1] // topconv_scale)
        self.pos_embed = self._pos_embed(dims[0], patch_size, imgsize) if posembed else None

        self.top_convs = nn.Sequential(ResidualBlock(in_chans, 48, 5),
        ...
        for i_layer in range(self.num_layers):
            downsample = self._make_downsample_v3(
                ...
                channel_first=self.channel_first,
                downsample_scale=downsample_scale[i_layer]
    ...
    def _pos_embed(embed_dims, patch_size, img_size):
        patch_height, patch_width = (img_size[0] // patch_size, img_size[1] // patch_size)
    ...
    @staticmethod
    def _make_downsample_v3(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False, downsample_scale=2):
        # if channel first, then Norm and Output are both channel_first
        return nn.Sequential(
            (nn.Identity() if channel_first else Permute(0, 3, 1, 2)),
            nn.Conv2d(dim, out_dim, kernel_size=downsample_scale, stride=downsample_scale, padding=0),
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),
            norm_layer(out_dim),
        )

    @staticmethod
    def _make_uplayer(dim=96, out_dim=192, norm_layer=nn.LayerNorm, channel_first=False, downsample_scale=2):
        if downsample_scale == 1:
            ...
            raise ValueError("The output down-sampling scale (ds_sacle[3]: int) should be 1, 2 or 3!")
    ...
    def forward(self, x: torch.Tensor):
        # print("Input:", x.shape)
        top_resx = self.top_convs(x)
        # print("Top:", top_resx.shape)
        x = self.patch_embed(top_resx)
        ...
        for layer in self.layers:
            # print("SSMLayer:", x.shape)
            x = layer.blocks(x)
            ssm_list.append(x)
            x = layer.downsample(x)
        # print("SSM:", ssm_list[0].shape, ssm_list[1].shape, ssm_list[2].shape)
        ssm_list[0] = self.patch_deconv(ssm_list[0])
        ssm_list[1] = self.ssm_up_layer1(ssm_list[1])
        ssm_list[2] = self.ssm_up_layer2(ssm_list[2])
        ssm_list.append(top_resx)
        # print("SSM + TopRes:", ssm_list[0].shape, ssm_list[1].shape, ssm_list[2].shape, ssm_list[3].shape)
        fuse_res = torch.cat(ssm_list, dim=1)
