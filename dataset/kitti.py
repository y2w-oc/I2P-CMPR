...
class KITTIDataset(Dataset):
    def __init__(self, seqs=['00'],
        ...
        for i in tqdm(range(len(self.raw_poses)), total=len(self.raw_poses), desc="Sequences: "):
            ...
            for j in tqdm(range(len(pose)), desc="Frames: "):
                ...
                key += 1
            acc_num += len(pose)
        print("Preprocessing complete! Total sample number:", len(self.pairs))
    ...
    def load_pcd(self, idx):
        ...
        # file = os.path.join(self.root, seq, "velodyne_in_cam", id + '.npy')
        # points_in_cam2 = np.load(file)
        return points[:, 0:3] #, points_in_cam2[:, 0:3]
    ...
    def __getitem__(self, idx):
        temp_seq_id = []
        temp_frame_id = []
        img_anchor = self.load_rgb_cam2(idx)

        pc = self.load_pcd(idx)

        # pc_hom = np.hstack((pc[:, :3], np.ones((pc.shape[0], 1))))
        # random_T = self.generate_random_transform()
        # pc_r = np.matmul(random_T, pc_hom.T).T
        # pc_r = pc_r[:,:3]
        # src_pc = o3d.geometry.PointCloud()
        # src_pc.points = o3d.utility.Vector3dVector(pc)
        # color1 = [23/255.0,190/255.0,207/255.0]
        # src_pc.colors = o3d.utility.Vector3dVector([color1]*len(src_pc.points))
        # r_pc = o3d.geometry.PointCloud()
        # r_pc.points = o3d.utility.Vector3dVector(pc_r)
        # color1 = [227 / 255.0, 119 / 255.0, 194 / 255.0]
        # r_pc.colors = o3d.utility.Vector3dVector([color1] * len(r_pc.points))
        # o3d.visualization.draw_geometries([src_pc, r_pc])

        if self.mode == 'test':
            pc_hom = np.hstack((pc[:, :3], np.ones((pc.shape[0], 1))))
            random_T = self.generate_random_transform()
            pc_hom = np.matmul(random_T, pc_hom.T).T
            pc = pc_hom[:, 0:3]
        ...
        # if idx == 1068:
        #     mask = range_anchor > 35
        #     range_anchor[mask] = 35
        #     plt.imshow(range_anchor, cmap="viridis")
        #     plt.axis("off")
        #     plt.savefig("range_anchor.png", bbox_inches="tight", pad_inches=0, dpi=600)
        #     plt.show()
        ...
        for i in range(len(pos_id)):
            ...
            pc_temp = self.load_pcd(idx_temp)
            ...
            # print(self.seqs[seq_id], frame_id)

            # print(temp_seq_id)
            # print(temp_frame_id)
            # print(rgb_pixels_train[:, 0:10])
...
class KITTI360Dataset(Dataset):
    def __init__(self, seq='02',
                 root="/media/yao/ssd-2T/KITTI-360/sequences",
                 mode='test'):
        super().__init__()
        self.seqs = seq
        self.root = root
        ...
        print("----------------------- Loading PNG file names ---------------------->")
        png_folder = os.path.join(self.root, self.seq, "image_00")
        png_files = os.listdir(png_folder)
        png_files.sort()
        print(len(png_files))

        print("----------------------- Loading Poses ---------------------->")
        pose_path = os.path.join(root.replace('sequences', 'data_poses'), seq + '.txt')
        pose = np.genfromtxt(pose_path)
        print(pose.shape)
        ...
        j = 0
        for i in range(pose.shape[0]):
            frame_id = int(pose[i,0])
            png_name = str(frame_id).zfill(10) + ".png"
            if png_name in png_files:
                pose_new.append(pose[i])
        pose_new = np.array(pose_new)
        print(pose_new.shape, len(png_files))
        self.pose = pose_new
    ...
    def load_pcd(self, idx):
        id = str(idx).zfill(10)
        file = os.path.join(self.root, self.seq, "velodyne", id + '.bin')
        points = np.fromfile(file, dtype='float32').reshape(-1, 4)
    ...
    def load_rgb_cam2(self, idx):
        id = str(idx).zfill(10)
        file = os.path.join(self.root, self.seq, "image_00", id + '.png')
        # print(file)
        img = cv2.imread(file)
        return img
    ...
    def __getitem__(self, idx):
        frame_id = self.pose[idx, 0]
        frame_id = int(frame_id)
        img_anchor = self.load_rgb_cam2(frame_id)

        pc = self.load_pcd(frame_id)

        range_anchor, pc, _, _ = range_projection(pc, return_others=True)

        img_anchor = self.crop_image(img_anchor)
...
if __name__ == '__main__':
    ...
    dataset = KITTIDataset(seqs=["00"], mode='test')
    for i in tqdm(range(1071, len(dataset))):
        data = dataset[i]
        # print(data['imgs'].shape, data['ranges'].shape, data['overlaps'].shape, data['range_pixels'].shape, data['rgb_pixels'].shape)

    # dataset = KITTI360Dataset(seq="09", mode='test')
